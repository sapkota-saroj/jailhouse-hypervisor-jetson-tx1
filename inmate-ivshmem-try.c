...
static u64 get_bar_sz(u16 bdf, u8 barn)
{
	...
	printk("PCI_CFG_BAR=%d \t PCI_CFG_BAR+8*barn=%d \n", PCI_CFG_BAR,(PCI_CFG_BAR+(8*barn)));
	...
	printk("bdf=%u \t barn=%u \t bar=%u \t tmp=%u \t barsz=%u \n", bdf, barn, bar, tmp, barsz);
	printk("bdf=%p \t barn=%p \t bar=%p \t tmp=%p \t barsz=%p \n", bdf, barn, bar, tmp, barsz);
	return barsz;
}
...
static int map_shmem_and_bars(struct ivshmem_dev_data *d)
{
   ...
   printk("shared memory size is %u || %p \n",d->shmemsz, d->shmemsz);
	...
	printk("Map range from=%p || %d to=%p bar2sz=%p || %u \n", d->shmem, d->shmem, d->shmemsz+PAGE_SIZE+d->bar2sz, d->bar2sz, d->bar2sz);
...
//	while (1) {
		for (i = 0; i < ndevices; i++) {
			...
			shmem = d->shmem;
			u32 x=12345;
			printk("writing to shared memory at %p with=%d \n", d->shmem,x);
			mmio_write32(d->shmem, x);
			//printk("writing to shared memory at %p with=%u || %p \n", d->shmem,mmio_read32(d->shmem),mmio_read32(d->shmem);
			shmem[19]++;
			//send_irq(d);
		}
